...
### 初回実装での失敗と、そこから決めた検証のやり方

初回実装はユニットテストが全て通った状態で「完了」としたが、実際に Chrome で動かすと
ブラウザ版の同期が**全く動かなかった**。原因は 2 つとも React のライフサイクルの扱いにあった。

1. **リソースを render 中に開いてはならない。** `BroadcastChannel` を `useState` の遅延初期化子で
   開き、`useEffect` の cleanup で閉じていた。`<StrictMode>` の開発ビルドでは初期化子が複数回呼ばれ、
   さらに setup → cleanup → setup が走る。結果、採用された 1 本が閉じられたまま復活せず
   (`postMessage` は `InvalidStateError`、`onmessage` は無反応)、余分な実体だけが listener を持って
   leak した。production ビルドでは二重実行が外れるため**たまたま動いてしまう**のが厄介だった。
   開く/閉じるは必ず同一 effect の setup / cleanup 対で持つこと。
2. **通知の失敗を保存の失敗に混ぜてはならない。** GDrive 側は `postMessage` の throw を保存と同じ
   `try` で受けており、catch が保存前の version を返すため、以降の保存が毎回偽の競合になっていた。

**テストのやり方も誤っていた。** 当時のテストはチャネルを React の外から直接叩いており、
壊れていたのは「React への配線」だったため素通りした。以降は次の 2 段構えを必須とする。

- `renderHook(..., { wrapper: StrictMode })` で**実際のマウント/再マウントを通した**テストを書く
  (`useLocalDocumentSync.test.ts`)。`StrictMode` はコンポーネント参照として渡せるので JSX は不要。
- 仕上げに**実ブラウザで 2 ウィンドウを開いて確認する**。環境に入っている playwright から
  同一 context の 2 ページを開き、片方でテーブルをドラッグしてもう片方が追従することを見る。
  修正前のコードに対して同じ手順が失敗することまで確認して、初めて直ったと言える。

この実ブラウザ確認の過程で `TitlePanel` の別問題も見つかった。入力欄がドキュメント名の複製を
初期値として持ち続けるため、他ウィンドウの改名が表示に反映されず、さらにその状態で入力欄を
離れるだけで古い名前を書き戻してしまっていた。編集中のみ値を持つ形に改めた。
