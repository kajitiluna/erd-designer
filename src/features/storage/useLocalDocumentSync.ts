...
const useLocalDocumentSync = ({
    ...
    // 生成は副作用を持たないため、StrictMode が初期化子を複数回呼んでも余分な実体はただのオブジェクトとして捨てられる。
    const [channel] = React.useState(() => {
        return new LocalDocumentSyncChannel(documentStorage, documentKey, erdDocument, initialRevision);
    });
    ...
    // cleanup だけを持つ effect にすると、StrictMode の setup → cleanup → setup で閉じたまま復活せず、
    // 他ウィンドウとの同期が一切効かなくなる。
    React.useEffect(() => {
