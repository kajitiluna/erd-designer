...
const VsCodeExtensionApplication = (prop: { vscodeApi: VsCodeApi }) => {
    ...
    // setter を呼ばないため実体は初回描画時から不変だが、
    // render 中に ref.current を読む (react-hooks/refs 違反) のを避けるため useState の遅延初期化で保持する。
    ...
    const handleSaveDocument = React.useCallback((erdDocument: ErdDocument, message: string) => {
        // 他パネル/他ウィンドウの保存や外部プロセスによる書き換えなど、
        // 既にファイルへ反映済みの変更を、そのまま拡張機能へ保存し返さない (MCP 経由の未反映の変更はこの対象に含まれない。
