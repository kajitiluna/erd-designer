...
type DrawableArea = { width: number, height: number };

const escapeHtml = (text: string): string =>
    text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");

const calculateImageArea = (erdCanvas: HTMLElement) => {
...
export const downloadHtml = (erdDocument: ErdDocument, drawableArea: DrawableArea) => {
    ...
    for (const memo of allMemos) {
        ...
        const contained = tableViewModels
            .filter(t => {
                const tx = t.corner.left + drawableArea.width / 2;
                const ty = t.corner.top + drawableArea.height / 2;
...
(function() {
  ...
  const MEMO_TABLES = ${memoTableJson};
