...
/**
 * 外部 (別ウィンドウ、Google Drive のリモート更新、VSCode 拡張機能側) から取り込んだ ErdDocument を MainView へ伝える。
 * ドキュメントの履歴管理は MainView 側の documentsHolder が一元的に担うため、
 ...
 * dispatch 中だけ取り込んだインスタンスを保持し、onSave に渡された引数がそれと同一参照であれば自分自身が発生させた保存だと判定できる。
 */
export default class ExternalDocumentChangeDispatcher {
