...
describe('VsCodeDocumentResource', () => {
    ...
    test('register した内容がそのまま findByUri / findById から取得できる', () => {
        ...
        resource.register(textDocument, toJsonContent(initialDocument), () => { });
    ...
    test('同一 URI への 2 回目の register は、既存のドキュメント内容を変更せずハンドラのみ追加する', () => {
        ...
        resource.register(textDocument, toJsonContent(initialDocument), () => { });
        resource.register(textDocument, toJsonContent(staleDocument), () => { });
    ...
    test('notify は同一 URI を開く全パネルのハンドラへ届く', () => {
        ...
        resource.register(textDocument, toJsonContent(initialDocument), updating => receivedByPanel1.push(updating));
        resource.register(textDocument, toJsonContent(initialDocument), updating => receivedByPanel2.push(updating));
    ...
    test('最後の 1 枚を閉じるとドキュメントが管理対象から外れる', () => {
        ...
        const unregisterPanel1 = resource.register(textDocument, toJsonContent(initialDocument), () => { });
        const unregisterPanel2 = resource.register(textDocument, toJsonContent(initialDocument), () => { });
    ...
    test('同じハンドラの重複解除は何もしない', () => {
        ...
        const unregister = resource.register(textDocument, toJsonContent(initialDocument), () => { });
